...
void matmul(const float *A, const float *B, float *C, int M, int N, int K) {
  CHECK_ERROR(clEnqueueWriteBuffer(queue, a_d, CL_TRUE, 0, M * K * sizeof(float), A, 0, NULL, NULL));
  CHECK_ERROR(clEnqueueWriteBuffer(queue, b_d, CL_TRUE, 0, K * N * sizeof(float), B, 0, NULL, NULL));
  ...
  // if(S%32 ==0)
  //  S = S -1;
  size_t global_size[2] = {S,S};

  //size_t global_size[2] = {M,N};
  //printf("Global Work Size: (%d, %d)\n", global_size[0], global_size[1]);
  size_t local_size[2] = {32, 32}; 

  CHECK_ERROR(clEnqueueNDRangeKernel(queue, kernel, 2, NULL, global_size, local_size, 0, NULL, NULL));
